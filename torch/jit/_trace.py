...
def trace(
    ...
    * The resulting recording of `nn.Module.forward` or `nn.Module` produces
    ...
    from torch._utils_internal import log_torchscript_usage, check_if_torch_exportable, log_torch_jit_trace_exportability
    ...
    if check_if_torch_exportable():
        ...
        export_args, _ = _process_jit_trace_inputs_for_export(example_inputs, example_kwarg_inputs)
        def _log_exportability(func_to_export, export_func, export_args, export_type):
            try:
                ...
                log_torch_jit_trace_exportability("trace", str(export_type), str(_ExportOutcome.FAILED_TO_EXPORT), str(e))
                return

            try:
                export = ep_module(*export_args)
            except Exception as e:
                log_torch_jit_trace_exportability("trace", str(export_type), str(_ExportOutcome.FAILED_TO_RUN), str(e))
                return

            traced_result = traced_func(*example_inputs)
            if not analyze_ts_result_with_export_result(export, traced_result):
                log_torch_jit_trace_exportability("trace", str(export_type), str(_ExportOutcome.ACCURACY_ERROR), "accuracy error")
                return

            log_torch_jit_trace_exportability("trace", str(export_type), str(_ExportOutcome.SUCCESS), "succeeded")
        ...
        if not isinstance(traced_func, torch.jit.ScriptModule):
            _log_exportability(traced_func, _direct_export_and_lower, _export_args, _ExportType.DIRECT_EXPORT)

        _log_exportability(traced_func, _convert_ts_to_export_experimental, export_args, _ExportType.TRACE_AND_EXPORT)
        _log_exportability(traced_func, _convert_ts_to_export_source_to_source, export_args, _ExportType.SOURCE_TO_SOURCE)
