...
Tensor _weight_int4pack_mm_mps(const Tensor& A, const Tensor& B, int64_t qGroupSize, const Tensor& qScaleAndZeros) {
  ...
  dispatch_sync_with_rethrow(mpsStream->queue(), ^() {
    @autoreleasepool {
      ...
      const auto maxThreadsPerGroup = static_cast<uint32_t>([quantizedPSO maxTotalThreadsPerThreadgroup]);
      constexpr uint32_t blockSize = 8;
      const uint32_t nBlockSize = std::min(maxThreadsPerGroup, blockSize);
      const uint32_t mBlockSize = std::min(maxThreadsPerGroup / nBlockSize, blockSize);
      ...
      [computeEncoder dispatchThreads:MTLSizeMake(N / 4, M / 4, 1)
                threadsPerThreadgroup:MTLSizeMake(nBlockSize, mBlockSize, 1)];
#if _CAPTURE_KERNEL
